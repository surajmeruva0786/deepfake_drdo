...
Each limitation maps to a direction. The \emph{generalization gap} invites a stronger audio backbone such as AASIST \cite{jung2022aasist}, at the cost of the symmetry that makes fusion recalibration-free. \emph{Calibration} needs post-hoc temperature scaling before any fused probability reaches a user. \emph{Thresholds} should be tuned against a deployment's relative cost of a missed forgery versus a false accusation, which the AND-gate's asymmetry (Section~VI-A) makes consequential. \emph{Scale} calls for zero-shot evaluation on an unseen corpus, LAV-DF \cite{cai2022lavdf} being the natural target, and for feature-level fusion, which the shared temporal pipeline already permits. \emph{Deployment} turns on PGCF's ResNet50 spatial stream: a mobile-class backbone would bring the pair within reach of on-device, real-time screening at a communications endpoint, subject to a latency and memory study we have not yet run.
