...
% Give stacked floats (e.g. two tables or two figures back-to-back in the
% same column) visible breathing room instead of sitting flush against
% each other or the surrounding text.
\setlength{\floatsep}{14pt plus 4pt minus 4pt}
\setlength{\textfloatsep}{18pt plus 4pt minus 4pt}
\setlength{\dblfloatsep}{14pt plus 4pt minus 4pt}
...
\title{PGCF+AGCF: Physiology- and Audio-Guided Consistency for Multi-Modal Deepfake Detection}
...
\begin{abstract}
Not every deepfake is visible in the video itself. In some forgeries the picture remains completely authentic and only the voice has been cloned or synthesized, a case that a detector examining pixels alone will always miss. We first develop the Physiology-Guided Consistency Framework (PGCF), a three-stream video detector that evaluates whether spatial detail, frequency patterns, and a physiological signal (remote photoplethysmography, or rPPG) are mutually consistent. PGCF performs well, but because it analyzes video only, it is structurally blind to audio-only attacks. This paper addresses that limitation by introducing an audio companion network, the Audio-Guided Consistency Framework (AGCF), designed as a deliberate mirror of PGCF. Both networks converge on the same temporal pipeline (Temporal Gradient Fusion, 4-head self-attention, BiLSTM(256), Classifier(512$\to$2)), so the two independently trained branches remain directly comparable. The front end of AGCF pairs a log-mel spectral CNN, analogous to PGCF's DCT frequency stream, with a raw-waveform 1D-CNN, analogous to PGCF's rPPG stream. We combine the two branches' outputs using two decision-level rules: a continuous Soft rule that multiplies their probabilities, and a stricter Hard AND-gate that classifies a clip as fake only when \emph{both} branches independently agree. On a held-out, identity-disjoint evaluation of the FakeAVCeleb corpus, PGCF achieves an AUC of 0.999 (98.5\% accuracy), AGCF achieves an AUC of 0.884 (78.8\% accuracy), Soft fusion achieves an AUC of 0.979 (93.4\% accuracy), and the Hard AND-gate achieves 100\% precision (zero false positives on real videos) at 83.8\% accuracy. We additionally report a data-integrity audit that identified and corrected two evaluation flaws -- identity leakage between the train and validation splits, and a filename-collision Cartesian merge that duplicated ``Real'' predictions -- and show that the uncorrected evaluation had inflated apparent performance, underscoring the importance of identity-disjoint protocols for audio-visual deepfake benchmarking. Robustness under additive Gaussian noise, ROC/PR analysis, calibration curves, and t-SNE feature-space visualization are reported for all four configurations.
\end{abstract}
...
As the first component of this work, we develop the Physiology-Guided Consistency Framework (PGCF), a three-stream video detector that evaluates the mutual consistency of spatial detail (ResNet50+CBAM), frequency patterns (DCT), and a physiological signal (CHROM rPPG) through a learned cross-modal consistency gate. On a balanced Celeb-DF-v2 validation set, PGCF achieves an AUC of 0.824. While effective against classical face-swap and re-enactment forgeries, PGCF operates on video alone and is therefore structurally blind to \emph{audio-driven} deepfakes -- most notably lip-sync attacks such as Wav2Lip \cite{prajwal2020wav2lip}, in which the video frames remain authentic and only the speech track is synthesized or cloned. Against such attacks, a video-only detector produces severe false negatives regardless of the sophistication of its visual reasoning, because the manipulation does not exist in the modality it examines.

This paper addresses that gap. We introduce the \textbf{Audio-Guided Consistency Framework (AGCF)}, an audio companion network designed as a stream-for-stream mirror of PGCF: a spectral CNN corresponds to PGCF's frequency stream, a raw-waveform 1D-CNN corresponds to PGCF's rPPG stream, and the resulting 512-d representation is processed through an \emph{identical} Temporal Gradient Fusion $\to$ 4-head self-attention $\to$ BiLSTM(256) $\to$ Classifier(512$\to$2) pipeline, trained with matching hyperparameters. Because the two branches share this structure, their output probabilities are directly comparable, allowing them to be fused at decision level -- via a continuous Soft (probability-product) rule, or a stricter Hard AND-gate that classifies a clip as fake only when both branches agree -- without additional recalibration. The AND-gate reduces the false-positive rate on real videos to zero, while PGCF alone continues to catch purely visual attacks and AGCF alone continues to catch the audio-only attacks that PGCF cannot detect.

Finally, while constructing the identity-disjoint evaluation protocol for AGCF on FakeAVCeleb \cite{khalid2021fakeavceleb}, we identified and corrected two data-integrity issues that had been affecting our reported metrics -- subject-level leakage between the train and validation splits, and a filename-collision bug that silently duplicated ``Real'' predictions during the merging of audio and video scores. We report both the uncorrected and corrected metrics side by side, since the magnitude of the difference is itself instructive for the audio-visual deepfake benchmarking community.

\textbf{Main contributions:}
\begin{itemize}
\item \textbf{AGCF}, a dual-stream (spectral and vocal) audio deepfake detector with a temporal backbone architecturally matched to PGCF's, enabling a fair, decision-level comparison between the two modalities.
...
\item A \textbf{comprehensive evaluation suite} (ROC/PR curves, confusion matrices, calibration curves, t-SNE visualization, and Gaussian-noise robustness analysis) for PGCF, AGCF, and both fusion strategies, together with a complete, code-level pipeline for \textbf{cross-dataset generalization} testing on LAV-DF \cite{cai2022lavdf}, presented as an engineering contribution with results reported as ongoing work rather than estimated.
\end{itemize}
...
\subsubsection{Spectral Stream}
Audio is resampled to 16~kHz and converted into a log-mel spectrogram (64 mel bins $\times$ 100 frames), processed by the same 3-layer 2D-CNN design used in PGCF's frequency stream (Table~\ref{tab:mirror}). This stream targets the unnatural spectral energy distributions and phase irregularities characteristic of vocoder- and TTS-generated speech -- the audio analogue of the artifacts PGCF's DCT stream targets in the visual domain.

\subsubsection{Vocal Stream}
The raw one-second waveform (16{,}000 samples) is processed directly in the time domain by a 1D-CNN mirroring the design of PGCF's rPPG stream (Table~\ref{tab:mirror}). This stream captures fine-grained vocal-tract detail -- pitch-period irregularities, robotic timbre, and phase discontinuities -- lost during spectrogram conversion, playing the same structural role for AGCF that the rPPG signal plays for PGCF: a signal a generative model is unlikely to reproduce faithfully, since it was not the target the generator was optimized against.
...
Both branches are trained independently, which keeps PGCF completely unmodified. At inference, each branch outputs an independent probability of forgery, and we evaluate two fusion rules.
...
\subsection{Datasets}
\textbf{Celeb-DF-v2} was used exclusively to train and validate the visual-only PGCF baseline (Section~III-A), providing a strong foundation of high-quality visual manipulations for spatial artifact learning.
...
\textbf{LAV-DF} \cite{cai2022lavdf} is used for cross-dataset generalization testing. We built an auto-resuming extraction pipeline for the 23.1~GB archive, a filtration script isolating a balanced 150-real/150-fake subset, and a zero-shot cross-evaluation script. This pipeline is a completed engineering contribution; the corresponding zero-shot numerical results were not finalized in time for this submission and are reported as ongoing work rather than estimated.
...
The audio branch exhibits the expected leakage effect: identity-disjoint evaluation lowers AGCF's accuracy and F1 relative to the naive split, consistent with partial memorization of speaker-specific voice characteristics. Notably, PGCF's identity-disjoint numbers are \emph{higher} than its naive-split numbers; since PGCF's spatial and frequency artifacts depend less on speaker identity than AGCF's vocal-tract features, this is more likely attributable to differing class balance and clip composition than to identity leakage itself. We report both splits transparently rather than assert a single causal explanation.
...
Table~\ref{tab:main} reports the primary, identity-disjoint evaluation for all four configurations. Fig.~\ref{fig:ablation} presents the same results as a bar chart, and Fig.~\ref{fig:forest} presents a per-metric comparison across all four.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/ablation_study.jpg}
\caption{Ablation study: AUC-ROC and accuracy contribution of each modality (identity-disjoint evaluation).}
\label{fig:ablation}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/metrics_unseen_forest.jpg}
\caption{Per-metric comparison across all four configurations (identity-disjoint evaluation).}
\label{fig:forest}
\end{figure}

Both fusion strategies achieve perfect precision (1.000) on this evaluation set -- zero real videos are misclassified as fake by either rule -- at the cost of recall relative to PGCF alone. The Hard AND-gate is the more conservative of the two, forfeiting 19.6 recall points relative to Soft fusion (0.673 vs. 0.867) in exchange for a simpler, threshold-free decision rule. ROC and Precision--Recall analysis confirm the same ordering: PGCF and Soft fusion maintain Average Precision above 0.98 (0.9988 and 0.9841, respectively) with a true-positive rate near 1.0 at a false-positive rate below 0.1, while AGCF's weaker AUC (Table~\ref{tab:main}) is echoed by a lower AP of 0.9009 that degrades further as recall exceeds roughly 0.5, reflecting its harder unseen-speaker generalization problem.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/confusion_matrices_unseen.jpg}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/robustness_noise.jpg}
...
By construction, the Hard AND-gate requires unanimous agreement, so its false-positive rate is upper-bounded by $\min(\text{FPR}_v, \text{FPR}_a)$: a real video is misclassified as fake only if \emph{both} branches independently err on the same clip. Since PGCF and AGCF process disjoint input signals (pixels vs. waveform) and train independently, their false-positive events are only weakly correlated, which is why the fused rate is empirically zero on both partitions. This confirms the paper's motivating hypothesis: PGCF is blind to audio-driven attacks where the visual stream is untouched (e.g., voice cloning over an authentic video), while AGCF is blind to visual-only attacks where the audio is untouched (e.g., a silent face-swap). Neither branch alone covers the full attack surface, and the recall lost under Hard AND fusion (Table~\ref{tab:main}) is the direct cost of requiring both modalities to agree -- a trade-off to select deliberately based on the relative cost of false accusations versus missed detections, not assumed to be cost-free.

\subsection{Limitations}
\textbf{Audio generalization gap.} AGCF's identity-disjoint AUC (0.884) trails PGCF's (0.999) and is markedly more noise-sensitive (Section~V-D); stronger audio-specific backbones (e.g., AASIST \cite{jung2022aasist}) could narrow this gap at the cost of breaking the architectural symmetry that makes fusion straightforward.

\textbf{Calibration.} PGCF and AGCF are both under-confident below a predicted probability of 0.6, and Soft fusion is visibly over-confident at low predicted probabilities, since a low-but-nonzero product of two probabilities can still correspond to a majority-fake sub-population; post-hoc calibration (e.g., temperature scaling) is recommended before any fused probability is surfaced as a user-facing confidence score.

\textbf{Fixed thresholds.} Both $\tau_v$ and $\tau_a$ are fixed at 0.5 throughout; a deployment-specific threshold search would likely yield a better precision-recall balance for the AND-gate.

\textbf{Dataset scale.} The identity-disjoint held-out set (198 clips) is small by modern standards; broader validation on larger corpora remains future work, alongside the LAV-DF cross-dataset evaluation.

\section{Conclusion}
We presented AGCF, an audio companion network architected as a stream-for-stream mirror of the PGCF video detector, and evaluated two decision-level fusion strategies for combining them. On an identity-disjoint, held-out FakeAVCeleb evaluation, PGCF alone achieves an AUC of 0.999, AGCF alone achieves 0.884, and a conservative Hard AND-gate achieves perfect precision (zero false positives) at 83.8\% accuracy, while Soft fusion recovers additional recall (0.867 vs. 0.673) at the same precision. We further documented and corrected an identity-leakage and Cartesian-merge evaluation bug, showing the resulting metric inflation is large enough to alter the reported picture of audio-branch performance. Future work will focus on closing AGCF's generalization and noise-robustness gap relative to PGCF, deployment-specific threshold tuning for the AND-gate, and completing the cross-dataset LAV-DF evaluation begun in this work.
...
\bibitem{cai2022lavdf} Z. Cai, K. Stefanov, A. Dhall, and M. Hayat, ``Do you really mean that? Content driven audio-visual deepfake dataset and multimodal method for temporal forgery localization,'' in \emph{2022 Int. Conf. Digital Image Computing: Techniques and Applications (DICTA)}, 2022, pp. 1--10.
\bibitem{jung2022aasist} J. Jung, H. Heo, H. Tak, H. Shim, J. S. Chung, B. Lee, H. Yu, and N. Evans, ``AASIST: Audio anti-spoofing using integrated spectro-temporal graph attention networks,'' in \emph{Proc. IEEE ICASSP}, 2022, pp. 6367--6371.
