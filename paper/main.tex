...
\begin{table}[t]
\caption{Dataset Used and Their Evaluation Protocol}
\label{tab:datasets}
\centering
\small
...
Requiring unanimity for the \emph{Real} verdict upper-bounds the AND-gate's miss rate by $\min(\text{FNR}_v, \text{FNR}_a)$: a forgery escapes only if \emph{both} branches fail to see it. Because PGCF and AGCF consume disjoint signals (pixels vs. waveform) and train independently, their misses are weakly correlated, so the fused rate tends toward the product of the two rather than the minimum. This is what the single-modality attacks of Section~III-D require, each being invisible to one branch and evident to the other. The symmetric cost is a fused false-positive rate bounded by $\text{FPR}_v + \text{FPR}_a$ rather than by either alone. In a monitoring role, where flagged material is escalated to a human analyst rather than acted on automatically, that balance favours recall -- the regime the AND-gate occupies.
